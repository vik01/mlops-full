...
from src.logit_regression.logit_infer import run_inference
...
def test_run_inference_output_format():
    ...
    preds_df = run_inference(model=model, X_infer=X_infer)

    assert isinstance(preds_df, pd.DataFrame)
    assert list(preds_df.columns) == ["prediction"]  # must be exactly one column with this name
